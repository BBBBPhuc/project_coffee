    ...
    public function createHoaDon(Request $request) {
        ...
        if ($user) {
            ...
            SendMailJob::dispatch($user->email, 'Thông Tin Thanh Toán', 'admin.pages.mail.mail_thanh_toan', $data);
                return response()->json([
                    'status'    => 1,
                    'message'   => 'Chúng tôi đã nhận thành công đơn hàng của bạn, vui lòng kiểm tra email',
                    'codeOrder' => $hoaDon
                ]);
        }
        } catch(Exception $e) {
    ...
    public function dataPay() {
        ...
        if ($user) {
            $hoaDon = HoaDonBanHang::where('nguoi_tao_hoa_don', $user->id)
                                   ->join('hoa_don_ban_hangs.ma_hoa_don', 'don_dat_mons.ma_hoa_don')
                                   ->join('hang_hoas.id', 'don_dat_mons.id_hang_hoa')
                                   ->select('hoa_don_ban_hangs.ma_hoa_don', 'hoa_don_ban_hangs.tong_tien', 'hoa_don_ban_hangs.is_thanh_toan',
                                   'hoa_don_ban_hangs.created_at', 'hoa_don_ban_hangs.updated_at','don_dat_mons.so_luong'
                                   )
                                   ->get();
